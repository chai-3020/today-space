...
### 12. 【高】云函数信任客户端传来的 `day`,可伪造任意历史日期(M1)

**问题**:`recordSession` 只校验 `day` 的格式,**从不与 `endedAt` 交叉验证** —— 反复调用并填任意历史日期,就能凭空造出连续多日的专注历史,污染统计。

**修复**:日期改由**服务端**从 `endedAt` + 客户端时区偏移推导:

- 客户端只传 `tzOffsetMinutes`(`new Date().getTimezoneOffset()`)与 `dayOffset`;
- `dayOffset` **只接受 0 或 -1**(给"午夜模式"归前一天用),其它值一律归一为 0 —— 所以客户端无法指定任意日期;
- 旧客户端若仍传 `day`,服务端只做一致性核对并记日志,实际以推导值为准。
- 验算覆盖 6 个场景(白天/23:59/00:30/午夜模式边界)与跨时区边界,全部通过。

### 13. 【中】统计改为服务端聚合(M6)

**问题**:四个页面各自 `focus_log.limit(1000).get()` 拉全量再在客户端按天累加 —— 每次进页面全量重拉,超过 1000 条会**静默截断、统计偏低**。

**修复**:新增云函数 `getFocusStats`,在服务端分页(每页 100,最多 30 页)聚合后只回传"每天一条"的小结果。`stats.js` / `index.js` / `todolist.js` / `pomodoro.js` 的今日专注与历史分布全部改走它;返回 `truncated` 标记,超上限时客户端会有 console 警告而不是无声偏小。

### 14. 【中】"清除已完成"改为服务端批量删除(B)

**问题**:客户端 `for-await` 逐条 `remove()`,N 条 = N 次网络往返,中途失败留下"删了一半"。

**修复**:新增云函数 `clearDoneTodos`,服务端按 `openid + done` 一次删除(云函数里权限规则不生效,因此显式带 `openid` 条件,避免误删他人数据),并回传实际删除条数。

### 15. 【中】便签单文档模型的三个缺陷(E/F/L4)

**问题**:① `docId` 为空时走 `add()`,首次保存"超时但服务端其实成功"会导致**幽灵文档**;② `onShow` 无条件 `loadNotes()` 会覆盖正在输入的本地内容;③ 600ms 防抖定时器在离开页面时没兑现,可能丢字。

**修复**:
- 改用**按用户确定性 `_id`**(`note_<openid尾16位>`)读写:`doc(id).get()` 判存在,`add({ _id })` 新建、已存在则退化为 `update` —— 每个用户永远只有一条,重复保存不产生副本;
- `_dirty` 标记:有未保存内容时 `onShow` 不再覆盖,并提示"有未保存的修改";
- `onHide` / `onUnload` 补存待保存内容并清理定时器。
...
> **本节的 H1–H5、M1、M2、M6 以及 A–G 各条均已修复**(见第一节第 4–16 条),下面保留的是**原始发现记录**,其中引用的行号与"当前行为"描述是修复前的状态,仅作追溯用。
> 现在仍未处理的只剩:`countdowns` 允许选过去日期、`stats` 时间轴标签在平移窗口后仍标"今天/本周"这类**文案级小瑕疵**,以及云函数 `addFocus`(已无调用方,仅保留)。
